...
const App = () => {
  ...
  // --- NEW: FETCH LIVE STOCK FROM JSON ---
  const handleCheckStock = async (hospital) => {
    try {
      // Adjust the path if your JSON is served from a different URL or backend endpoint
      const response = await fetch('/live_data/hospitals.json');
      ...
      // Find the specific hospital in the JSON file
      const liveHospitalStock = liveData.find(h => h.id === hospital.id);

      if (liveHospitalStock) {
        // Update selectedHospital with the freshly fetched live data
        setSelectedHospital({ ...hospital, ...liveHospitalStock });
      } else {
        console.warn("Node not found in live stream. Falling back to local cache.");
        setSelectedHospital(hospital);
      }
    } catch (error) {
      console.error("Live fetch error:", error);
      setSelectedHospital(hospital); // Fallback to initial local state
    }
  };
            ...
            <table className="w-full text-left border-collapse bg-white">
              ...
              <tbody>
                {displayedHospitals.length > 0 ? displayedHospitals.map((h, index) => (
                  <tr key={h.id} onClick={() => handleCheckStock(h)} className="border-b border-neutral-200 hover:bg-red-50 group cursor-pointer transition-colors">
                    ...
                    <td className="p-5 text-right">
                      <button
                        ...
                        className="px-4 py-2 bg-black text-white text-[10px] font-black uppercase tracking-widest rounded-lg group-hover:bg-red-600 transition-colors"
                      >
                        Check Stock
                      </button>
                    </td>
